...
class Trainer(object, metaclass=abc.ABCMeta):
    ...
    def try_evaluate(self, epoch):
        ...
        if self.train_epoch % args.eval_interval == 0:
            ...
            self.logger.add_scalar('val/loss', float(vl), self.train_epoch)
            self.logger.add_scalar('val/acc', float(va),  self.train_epoch)
            print('epoch {}, val, loss={:.4f} acc={:.4f}+{:.4f}'.format(epoch, vl, va, vap))

            # if epoch == 80:
            #     self.trlog['max_acc'] -= 0.002
    ...
    def get_parameter_num(self):
        ...
        trainable_sum  = sum(p.numel() for p in self.model.parameters() if p.requires_grad)
