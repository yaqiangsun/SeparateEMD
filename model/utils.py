...
def predict(logits, label):
    pred = torch.argmax(logits, dim=1)
    if torch.cuda.is_available():
        return pred.type(torch.cuda.FloatTensor)
    else:
        return pred.type(torch.FloatTensor)
...
def postprocess_args(args):            
    ...
    if not args.augment:
        save_path2 += '-NoAug'
    save_path2 += args.loss
    save_path2 += args.save_flag
...
def get_command_line_parser():
    ...
    parser.add_argument('--num_test_episodes', type=int, default=5000)
    parser.add_argument('--model_class', type=str, default='FEAT', 
                        choices=['ProtoNet','SeparateEMD']) # None for MatchNet or ProtoNet
    ...
    parser.add_argument('--not_random_val_task',action='store_true', default=False,help='random samples tasks for validation at each epoch')
    # DEEPEMD slvoer about
    parser.add_argument('-solver', type=str, default='opencv', choices=['opencv', 'qpth'])
    ...
    parser.add_argument('--no_avg_pool', action='store_true', default=False)     # means do not average the featuremap
    ...
    parser.add_argument('--save_flag', type=str, default='None')
