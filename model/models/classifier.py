...
class Classifier(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.learnable_scale_encoder = torch.nn.Parameter(torch.FloatTensor(1).fill_(1.0),requires_grad=False)
        self.learnable_scale_logit = torch.nn.Parameter(torch.FloatTensor(1).fill_(1.0),requires_grad=False)

    def _init_weights(self, m):
        if isinstance(m, nn.Linear):
            from timm.models.layers import trunc_normal_
            trunc_normal_(m.weight, std=.02)#.02
