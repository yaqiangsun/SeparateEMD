...
class FewShotModel(nn.Module):
    def __init__(self, args):
        ...
        if args.backbone_class == 'ConvNet':
            ...
            # self.encoder = ResNet()
            if args.deepemd == "fcn":
                self.encoder = ResNet(avg_pool=(not self.args.no_avg_pool))
        ...
        self.learnable_scale_attention = None
        self.learnable_scale_relative_attn = None
    ...
    def forward(self, x, get_feature=False):
        if get_feature:
            ...
            x = x.squeeze(0)
            input_size = x.size()
            if self.args.deepemd == 'sampling':
                x = x.view(-1,input_size[2],input_size[3],input_size[4])
            instance_embs = self.encoder(x)
            if self.args.deepemd == 'sampling':
                instance_embs = instance_embs.view(input_size[0],input_size[1],instance_embs.size()[-1])
                instance_embs = instance_embs.transpose(1,2)
            # num_inst = instance_embs.shape[0]
            # split support query set for few-shot data
            support_idx, query_idx = self.split_instances(x)
            if self.training:
                logits, logits_reg,proto_dist = self._forward(instance_embs, support_idx, query_idx)
